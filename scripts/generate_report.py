...
from csp_analyzer import TimetableCSP
...
def main():
    ...
    print("\n[2/5] Running Optimization & Calculating KPIs...")
    all_kpis = []
    
    for scenario in ['baseline', 'scenario_a', 'scenario_b']:
        print(f"\n  - {scenario}...")
        
        # 1. First, run the optimizer to reschedule displaced events
        print("    Running CSP optimizer...")
        csp_optimizer = TimetableCSP(scenario)
        csp_optimizer.check_hard_constraints()
        csp_optimizer.greedy_reschedule()
        csp_optimizer.simulated_annealing()
        
        # 2. Extract final hard constraint summary from CSP
        final_result = csp_optimizer.check_hard_constraints()
        print(f"    Optimizer finished. Final unscheduled: {final_result.events_unscheduled:,}")
        
        # 3. Calculate all KPIs using the optimized schedule
        print("    Calculating full KPI suite...")
        calc = KPICalculator(scenario, csp=csp_optimizer)
        kpis = calc.calculate_all_kpis()
        
        # Sync the KPI 'feasibility' dict so the final report accurately reflects
        # what the CSP accomplished, replacing the initial raw-data values.
        kpis['feasibility']['unscheduled_events'] = final_result.events_unscheduled
        kpis['feasibility']['capacity_violations'] = final_result.capacity_violations
        kpis['feasibility']['compulsory_clashes'] = final_result.clash_count
        kpis['feasibility']['is_feasible'] = final_result.is_feasible
        
        all_kpis.append(kpis)
        
        print(f"    Final Unscheduled: {kpis['feasibility']['unscheduled_events']:,}")
        print(f"    Lunch break compliance: {kpis['student_experience']['lunch_break_percentage']}%")
